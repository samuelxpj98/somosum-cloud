...
const Header: React.FC<HeaderProps> = ({ content }) => {
  ...
  useEffect(() => {
    const storedSessions = localStorage.getItem('app_sessions');
    const count = storedSessions ? parseInt(storedSessions) : 0;
    
    if (!sessionStorage.getItem('session_counted')) {
      const newCount = count + 1;
      localStorage.setItem('app_sessions', newCount.toString());
