...
const Profile: React.FC<ProfileProps> = ({ profile, onUpdate, readCount, userPosts }) => {
  ...
  // Filtrar posts salvos (dos estáticos e dos criados pelo usuário)
    ...
    <div className={`min-h-screen pb-32 transition-colors duration-300 ${isDark ? 'bg-slate-900' : 'bg-slate-50'}`}>
      ...
      <main className="px-6 pt-8 space-y-8 animate-in fade-in duration-500">
          ...
          {isEditing ? (
            <div className="w-full space-y-4 max-w-xs mx-auto">
              <div className="space-y-1 text-left">
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-2">Nome</label>
                <input 
            ...
            <>
              ...
              <div className="flex items-center gap-2 mt-3 bg-blue-600/10 px-3 py-1 rounded-full border border-blue-600/20">
                 ...
                 <span className="text-[10px] font-black text-blue-600 uppercase tracking-widest">Membro Verificado</span>
              </div>
            </>
          )}
        </section>
        ...
        <section className="grid grid-cols-3 gap-3">
          <div className={`${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'} p-4 rounded-3xl shadow-sm border flex flex-col items-center gap-1 transition-colors`}>
            ...
            <span className={`text-xl font-black ${isDark ? 'text-white' : 'text-slate-900'}`}>{profile.stats.daysInRow}</span>
            <span className="text-[8px] font-bold text-slate-400 uppercase tracking-wider text-center">Dias Seguidos</span>
          </div>
          <div className={`${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'} p-4 rounded-3xl shadow-sm border flex flex-col items-center gap-1 transition-colors`}>
            <span className="material-symbols-outlined text-indigo-500 text-2xl">menu_book</span>
            <span className={`text-xl font-black ${isDark ? 'text-white' : 'text-slate-900'}`}>{readCount}</span>
            <span className="text-[8px] font-bold text-slate-400 uppercase tracking-wider text-center">Lidos</span>
          </div>
          <div className={`${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'} p-4 rounded-3xl shadow-sm border flex flex-col items-center gap-1 transition-colors`}>
            <span className="material-symbols-outlined text-emerald-500 text-2xl">bookmark</span>
            <span className={`text-xl font-black ${isDark ? 'text-white' : 'text-slate-900'}`}>{profile.stats.savedPosts}</span>
            <span className="text-[8px] font-bold text-slate-400 uppercase tracking-wider text-center">Salvos</span>
          </div>
        </section>

        {/* Leituras Salvas Section */}
        <section className="space-y-4">
          <div className="flex items-center justify-between px-2">
            <h3 className={`text-xl font-black font-display tracking-tight ${isDark ? 'text-white' : 'text-slate-900'}`}>Biblioteca Offline</h3>
          ...
          <div className="grid gap-4">
            {savedPosts.map(post => (
              <div 
                ...
                className={`group p-4 rounded-[28px] border transition-all active:scale-[0.98] flex items-center gap-4 ${isDark ? 'bg-slate-800 border-slate-700 hover:bg-slate-700' : 'bg-white border-slate-100 hover:shadow-md'}`}
              >
            ...
            {savedPosts.length === 0 && (
              <div className={`p-12 rounded-[32px] border-2 border-dashed flex flex-col items-center justify-center gap-3 opacity-40 ${isDark ? 'border-slate-800' : 'border-slate-200'}`}>
                ...
                <p className="text-xs font-bold uppercase tracking-widest text-center">Sua biblioteca está vazia.<br/>Salve artigos para ler depois.</p>
                <button onClick={() => navigate('/aprofundar')} className="text-blue-600 text-[10px] font-black uppercase underline">Explorar Artigos</button>
              </div>
            )}
          </div>
        </section>
        ...
        <section className={`p-6 rounded-[32px] border ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'} space-y-6 transition-colors shadow-sm`}>
          ...
          <div className="flex items-center gap-3">
             ...
             <div className="flex-1">
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Formação / Universidade</p>
                {isEditing ? (
                  <div className="space-y-3 mt-1">
                    <input 
                      value={editedProfile.education}
                      onChange={e => setEditedProfile({...editedProfile, education: e.target.value})}
                      placeholder="Ex: Cursando Universidade"
                    ...
                    <input 
                      ...
                      className={`w-full font-bold text-xs focus:outline-none bg-transparent border-b border-blue-500/20 pb-1 ${isDark ? 'text-blue-400' : 'text-blue-600'}`}
                    />
                  </div>
                ) : (
                  <>
                    <p className={`font-bold text-sm ${isDark ? 'text-white' : 'text-slate-900'}`}>{profile.education || 'Não informado'}</p>
                    {profile.course && <p className="text-blue-600 text-xs font-bold">{profile.course}</p>}
                  </>
                )}
             </div>
          </div>
        ...
        <div className="space-y-4">
          {isEditing ? (
            <div className="flex gap-4 animate-in slide-in-from-bottom-4 duration-300">
              <button 
                ...
                className={`flex-1 h-16 rounded-[24px] font-black uppercase text-xs tracking-widest border ${isDark ? 'bg-slate-800 border-slate-700 text-slate-400' : 'bg-white border-slate-100 text-slate-400'}`}
              >
                Cancelar
              </button>
              <button 
                onClick={handleSave}
                className="flex-[2] h-16 bg-blue-600 text-white rounded-[24px] font-black uppercase text-xs tracking-widest shadow-xl shadow-blue-600/20 active:scale-95 transition-transform"
              >
                Salvar Alterações
              </button>
            </div>
            ...
            <button 
              ...
              className={`w-full h-16 rounded-[24px] font-black uppercase text-xs tracking-widest border shadow-sm flex items-center justify-center gap-2 transition-all active:scale-[0.98] ${isDark ? 'bg-slate-800 border-slate-700 text-white' : 'bg-white border-slate-100 text-slate-900'}`}
            >
          ...
          <button className={`w-full h-16 border rounded-[24px] font-black uppercase text-xs tracking-widest flex items-center justify-center gap-2 transition-colors ${isDark ? 'bg-red-500/10 border-red-500/20 text-red-500' : 'bg-white border-red-100 text-red-500'}`}>
